...
func (vc *vaultClient) setK8SRole(k8sPath, roleName string, roleReq createRoleRequest) error {
	url := fmt.Sprintf("%s/v1/auth/%s/role/%s", vc.addr, k8sPath, roleName)

	b, err := json.Marshal(roleReq)
	if err != nil {
		return err
	}
